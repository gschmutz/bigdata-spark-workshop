...
```bash
docker run --name bluesky-retriever --rm -d -e DESTINATION=kafka -e KAFKA_BROKERS=127.0.0.1:9092 -e KAFKA_TOPIC=bluesky.raw ghcr.io/gschmutz/bluebird:latest
```

Let's see the messages comming in with `kcat` 

```bash
kca	t -q -b 127.0.0.1:9092 -t bluesky.raw
```

What are the different message types?

```bash
kcat -q -b 127.0.0.1:9092 -t bluesky.raw | jq .record.commit.collection
```

Let's view only the `text` of an `app.bsky.feed.post` message

```bash
kcat -q -b 127.0.0.1:9092 -t bluesky.raw | jq 'select(.record.commit.collection == "app.bsky.feed.post") | .record.commit.record.text'
```
...
```bash
curl -X PUT \
  http://127.0.0.1:8083/connectors/elasticsearch-bluesky-sink/config \
...
Visulalize the connector in the [Kafka Connect UI](http://127.0.0.1:28103/).
