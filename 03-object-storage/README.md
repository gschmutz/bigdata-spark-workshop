...
Enter `<useralias>-bhf-flight-bucket` into the **Bucket name** field. Make sure to replace <useralias> by your alias. Remember, bucket names have to be globally unique.
...
To upload a file we can use the browser-based UI. 

are going to use the `s3cmd put` command. First for the `airports.csv`
