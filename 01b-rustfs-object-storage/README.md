...
> **What you should see:** The RustFS Console browser view showing an empty list of buckets (or the `admin-bucket` if the platform has already initialised it).
...
> **What you should see:** The new `flight-bucket` appears in the bucket list in the Browser view.
...
> **What you should see:** The line `Bucket created successfully \`rustfs-1/flight-bucket\`` confirming the bucket was created.
...
> **What just happened?** RustFS, like S3, has no native directory concept. The `raw/airports/airports.csv` key simply contains a `/` separator, and `mc ls` groups objects by their common prefix to give a familiar folder-like view.
...
> **What you should see:** A progress bar and confirmation that the PDF was uploaded to `rustfs-1/flight-bucket/raw/pdf/pilot_handbook.pdf`.
...
> **What just happened?** The copy happens server-side inside RustFS — no data is transferred to your machine and back. This is equivalent to S3's server-side copy operation.
...
> **What you should see:** A confirmation line `Removed \`rustfs-1/flight-bucket/raw/airports/airports.csv\``.
...
> **What you should see:** `Removed \`rustfs-1/flight-bucket\``. If the bucket still contains objects the command will fail with an error — empty the bucket first or use `--force`.
...
> **What you should see:** `Removed \`rustfs-1/flight-bucket\`` after all contained objects have been deleted.
